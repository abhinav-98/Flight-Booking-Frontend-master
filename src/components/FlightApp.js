...
const FlightApp = () => {
  ...
  // handle submit
  const airports=[
    'Mumbai (BOM)','Pune (PNQ)','Delhi (DEL)','Bengaluru (BLR)'
  ]
  const filterFlights=(data)=>{
    const output=FlightsData.filter((ds)=>{
      if(ds.origin===data.departure && ds.destination===data.arrival){
        return ds;
      }
    });
    return output;
  ...
  const onSubmit = (data) => {
    ...
    navigate('/searchFlight/flights',{state:{data:filterFlights(data)}});
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {airports.map((airport)=>{
                          return(
                            <option key={airport.toString()} value={airport}>{airport}</option>
                          )
                        })}
                      </select>
